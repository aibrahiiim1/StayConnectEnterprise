# Changed-file manifest (generated - do not hand-edit)

- **Base commit:** `1fee3a57477573ec1093f836ea14baca1031f442`
- **HEAD commit:** `47870d98`
- **Provenance (generation HEAD = inventory_head):** `47870d989f412321a93e8e0783341e5667d616de`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
- **Branch:** `fix/sandbox-paths-routing-and-login-design`
- **Remote branch:** `origin/fix/sandbox-paths-routing-and-login-design`
- **Changed files:** 39
- **Generated by:** `tools/generate-change-manifest.py 1fee3a574775..STAGED`

## Files

| Path | Classification | Git status | Domain | Workstream | Rollback | Purpose (last commit subject in range) |
|---|---|---|---|---|---|---|
| `data-plane/cmd/edged/resources_portal_assets.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | Provision the paths the operator surfaces write, in the repo rather than by hand |
| `data-plane/cmd/portald/landing_design_test.go` | MODIFIED | `M` | tests/tooling | RUNTIME | rollback RESTORES prior content | The login page now matches the reference, structurally and not just visually |
| `data-plane/cmd/portald/templates.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The login page now matches the reference, structurally and not just visually |
| `data-plane/cmd/scd/backup_settings.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | Write the retention policy in place rather than widening the sandbox |
| `data-plane/cmd/scd/main.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The settings handlers were mounted on nothing |
| `data-plane/cmd/scd/routes_reachable_test.go` | CREATED | `A` | tests/tooling | RUNTIME | rollback REMOVES it | The settings handlers were mounted on nothing |
| `deploy/systemd/stayconnect-edged.service` | MODIFIED | `M` | configuration | DEPLOY | rollback RESTORES prior content | Provision the paths the operator surfaces write, in the repo rather than by hand |
| `deploy/systemd/stayconnect-scd.service` | MODIFIED | `M` | configuration | DEPLOY | rollback RESTORES prior content | Provision the paths the operator surfaces write, in the repo rather than by hand |
| `deploy/tmpfiles/stayconnect-operator-surfaces.conf` | CREATED | `A` | configuration | DEPLOY | rollback REMOVES it | Provision the paths the operator surfaces write, in the repo rather than by hand |
| `docs/architecture/StayConnect-IAM-Phase0-Contract.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `docs/architecture/StayConnect-IAM-Phase1A-Plan.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `docs/architecture/StayConnect-IAM-Phase1B-Plan.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `docs/context/StayConnect-IAM-Handoff.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `docs/manifests/PostClosure-change-manifest.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Delivery: manifest lists itself |
| `exports/chatgpt/StayConnectEnterprise-ChatGPT-Project-Pack.zip` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/StayConnectEnterprise-Phase-Evidence-Pack.zip` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/StayConnectEnterprise-Phase1B-Planning-Pack.zip` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase-evidence/GIT_STAT_79b2e930.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | Delivery: packs resynchronized |
| `exports/chatgpt/phase-evidence/GIT_STAT_93292ad6.txt` | EXPORTED | `D` | export | EXPORT | rollback RESTORES it | Governance: T0157 records what the sandboxes and the router caught |
| `exports/chatgpt/phase-evidence/PACK_SHA256SUMS.txt` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase-evidence/REPOSITORY_ARTIFACT_SHA256SUMS.txt` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase1b-planning/MANIFEST.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase1b-planning/PACK_SHA256SUMS.txt` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase1b-planning/REPOSITORY_ARTIFACT_SHA256SUMS.txt` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/phase1b-planning/StayConnect-IAM-Phase1B-Plan.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/00-START-HERE.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/MANIFEST.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Delivery: packs resynchronized |
| `exports/chatgpt/stayconnectenterprise/PROJECT-INSTRUCTIONS.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/StayConnect-IAM-Handoff.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/StayConnect-IAM-Phase0-Contract.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/StayConnect-IAM-Phase1A-Plan.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `exports/chatgpt/stayconnectenterprise/StayConnect-IAM-Phase1B-Plan.md` | EXPORTED | `M` | export | EXPORT | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `governance/project-state.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | Delivery: packs resynchronized |
| `governance/transitions/T0157.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | Governance: T0158 records what the sandboxes and the router caught |
| `governance/transitions/T0158.json` | CREATED | `A` | governance | GOVERNANCE | rollback REMOVES it | Governance: T0158 also records the login-page rebuild |
| `hotel-admin/app/(app)/portal-branding/page.tsx` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The login page now matches the reference, structurally and not just visually |
| `hotel-admin/e2e/phase3-guest-portal-resilience.spec.ts` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The login page now matches the reference, structurally and not just visually |
| `hotel-admin/e2e/phase3-guest-portal.spec.ts` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The legacy-endpoint test asserts on a request it never waited for |
| `hotel-admin/test/backup-settings-and-assets.test.tsx` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | The login page now matches the reference, structurally and not just visually |

## Total diff statistics (`git diff --stat`)
```text
 data-plane/cmd/edged/resources_portal_assets.go    |   8 +-
 data-plane/cmd/portald/landing_design_test.go      |  70 ++++++++-
 data-plane/cmd/portald/templates.go                | 102 +++++++++----
 data-plane/cmd/scd/backup_settings.go              |  23 +--
 data-plane/cmd/scd/main.go                         |   2 +
 data-plane/cmd/scd/routes_reachable_test.go        |  77 ++++++++++
 deploy/systemd/stayconnect-edged.service           |   6 +
 deploy/systemd/stayconnect-scd.service             |  11 ++
 deploy/tmpfiles/stayconnect-operator-surfaces.conf |  37 +++++
 .../StayConnect-IAM-Phase0-Contract.md             |   2 +-
 docs/architecture/StayConnect-IAM-Phase1A-Plan.md  |   2 +-
 docs/architecture/StayConnect-IAM-Phase1B-Plan.md  |   2 +-
 docs/context/StayConnect-IAM-Handoff.md            |   2 +-
 docs/manifests/PostClosure-change-manifest.md      | 159 +++++++++++----------
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip | Bin 340767 -> 340769 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  | Bin 140420 -> 140432 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip | Bin 43968 -> 43971 bytes
 .../chatgpt/phase-evidence/GIT_STAT_79b2e930.txt   |   4 +
 .../chatgpt/phase-evidence/GIT_STAT_93292ad6.txt   |   4 -
 exports/chatgpt/phase-evidence/PACK_SHA256SUMS.txt |   4 +-
 .../REPOSITORY_ARTIFACT_SHA256SUMS.txt             |   4 +-
 exports/chatgpt/phase1b-planning/MANIFEST.md       |   2 +-
 .../chatgpt/phase1b-planning/PACK_SHA256SUMS.txt   |   6 +-
 .../REPOSITORY_ARTIFACT_SHA256SUMS.txt             |   6 +-
 .../StayConnect-IAM-Phase1B-Plan.md                |   2 +-
 .../chatgpt/stayconnectenterprise/00-START-HERE.md |   2 +-
 exports/chatgpt/stayconnectenterprise/MANIFEST.md  |  60 ++++----
 .../stayconnectenterprise/PROJECT-INSTRUCTIONS.md  |   2 +-
 .../StayConnect-IAM-Handoff.md                     |   2 +-
 .../StayConnect-IAM-Phase0-Contract.md             |   2 +-
 .../StayConnect-IAM-Phase1A-Plan.md                |   2 +-
 .../StayConnect-IAM-Phase1B-Plan.md                |   2 +-
 governance/project-state.json                      |  14 +-
 governance/transitions/T0157.json                  |   3 +-
 governance/transitions/T0158.json                  |  54 +++++++
 hotel-admin/app/(app)/portal-branding/page.tsx     |   9 +-
 .../e2e/phase3-guest-portal-resilience.spec.ts     |  20 +--
 hotel-admin/e2e/phase3-guest-portal.spec.ts        |  11 +-
 .../test/backup-settings-and-assets.test.tsx       |   7 +-
 39 files changed, 526 insertions(+), 199 deletions(-)
```

## Working-tree status (`git status --short --untracked-files=all`)
```text
M  governance/project-state.json
```

## Commits in range (`git log --oneline <base>..HEAD`)
```text
HISTORICAL: 47870d98 Delivery: packs resynchronized
HISTORICAL: 79b2e930 The legacy-endpoint test asserts on a request it never waited for
HISTORICAL: 89ccc9dc Delivery: manifest lists itself
HISTORICAL: c3151697 Delivery: packs resynchronized
HISTORICAL: 4ab44271 Governance: T0158 also records the login-page rebuild
HISTORICAL: 2609876d The login page now matches the reference, structurally and not just visually
HISTORICAL: 146d1c0f Delivery: head pointer and manifest aligned
HISTORICAL: d2a78ad6 Delivery: manifest lists itself
HISTORICAL: c4fb2dea Governance: T0158 records what the sandboxes and the router caught
HISTORICAL: 58e4b20e Delivery: manifest lists itself
HISTORICAL: d289df00 Governance: T0157 records what the sandboxes and the router caught
HISTORICAL: e00f2571 Provision the paths the operator surfaces write, in the repo rather than by hand
HISTORICAL: cf81174b Write the retention policy in place rather than widening the sandbox
HISTORICAL: 17941550 The settings handlers were mounted on nothing
```
