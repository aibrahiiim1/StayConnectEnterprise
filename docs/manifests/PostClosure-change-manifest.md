...
- **Base commit:** `e0d7df745b0c624c2b27f9d4c4df3c7b666a4559`
- **HEAD commit:** `a759ab08`
- **Provenance (generation HEAD = inventory_head):** `a759ab08a4335a46f4b92ca42164079e770e7138`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
- **Branch:** `fix/pms-history-provenance-join`
- **Remote branch:** `(no upstream)`
- **Changed files:** 28
- **Generated by:** `tools/generate-change-manifest.py e0d7df745b0c..STAGED`

## Files

| Path | Classification | Git status | Domain | Workstream | Rollback | Purpose (last commit subject in range) |
|---|---|---|---|---|---|---|
| `data-plane/cmd/edged/phase3_revision_provenance_test.go` | CREATED | `A` | tests/tooling | RUNTIME | rollback REMOVES it | History said every version's origin was unrecorded; it was the join |
| `data-plane/cmd/edged/resources_phase3_interfaces.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | History said every version's origin was unrecorded; it was the join |
...
| `exports/chatgpt/phase-evidence/GIT_STAT_1c78ea25.txt` | EXPORTED | `D` | export | EXPORT | rollback RESTORES it | (no commit subject in range) |
| `exports/chatgpt/phase-evidence/GIT_STAT_a759ab08.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | (no commit subject in range) |
...
| `governance/transitions/T0134.json` | CREATED | `A` | governance | GOVERNANCE | rollback REMOVES it | History said every version's origin was unrecorded; it was the join |
| `tools/preflight.sh` | MODIFIED | `M` | tests/tooling | TOOLING | rollback RESTORES prior content | Preflight ran its own copy of the receipt rule, and the copy was wrong |

## Total diff statistics (`git diff --stat`)
```text
 .../cmd/edged/phase3_revision_provenance_test.go   | 156 +++++++++++++++++++++
 .../cmd/edged/resources_phase3_interfaces.go       |  45 ++++--
 ...
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip | Bin 338982 -> 338981 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  | Bin 139871 -> 139892 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip | Bin 43739 -> 43738 bytes
 ...
 governance/transitions/T0134.json                  |  66 +++++++++
 tools/preflight.sh                                 |  40 ++----
 28 files changed, 398 insertions(+), 168 deletions(-)
```
...
D  exports/chatgpt/phase-evidence/GIT_STAT_1c78ea25.txt
A  exports/chatgpt/phase-evidence/GIT_STAT_a759ab08.txt
...
```text
HISTORICAL: a759ab08 Preflight ran its own copy of the receipt rule, and the copy was wrong
HISTORICAL: aa34386c History said every version's origin was unrecorded; it was the join
```
