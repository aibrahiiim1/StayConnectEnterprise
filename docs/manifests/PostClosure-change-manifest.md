...
- **Provenance (generation HEAD = inventory_head):** `97aec315cc9d834fb40cf372ef8da399630884d2`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
...
| `docs/manifests/PostClosure-change-manifest.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | Governance: T0149 records the real-path reinstatement delivery deployed and verified |
...
```text
 ...
 docs/manifests/PostClosure-change-manifest.md      | 118 ++++++++-------------
 ...
 25 files changed, 167 insertions(+), 137 deletions(-)
```
