...
- **Base commit:** `af65e18ae7aa1fe885b9e6e6ed38456745682621`
- **HEAD commit:** `d98da735`
- **Provenance (generation HEAD = inventory_head):** `d98da73551466d4bf56040406a5e4b4544b50262`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
- **Branch:** `feat/checkout-grace-effective-policy-and-room-reuse-matrix`
- **Remote branch:** `(no upstream)`
- **Changed files:** 29
- **Generated by:** `tools/generate-change-manifest.py af65e18ae7aa..STAGED`

## Files

| Path | Classification | Git status | Domain | Workstream | Rollback | Purpose (last commit subject in range) |
|---|---|---|---|---|---|---|
| `data-plane/cmd/edged/resources_phase3.go` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | A room is not a guest, and the screen now says which grace is actually in force |
| `data-plane/internal/checkout/room_reuse_and_lifecycle_matrix_integration_test.go` | CREATED | `A` | tests/tooling | RUNTIME | rollback REMOVES it | A room is not a guest, and the screen now says which grace is actually in force |
...
| `exports/chatgpt/phase-evidence/GIT_STAT_cb8be71d.txt` | EXPORTED | `D` | export | EXPORT | rollback RESTORES it | (no commit subject in range) |
| `exports/chatgpt/phase-evidence/GIT_STAT_d98da735.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | (no commit subject in range) |
...
| `governance/project-state.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | A room is not a guest, and the screen now says which grace is actually in force |
| `governance/transitions/T0144.json` | CREATED | `A` | governance | GOVERNANCE | rollback REMOVES it | A room is not a guest, and the screen now says which grace is actually in force |
| `hotel-admin/components/phase3/checkout-grace-form.tsx` | MODIFIED | `M` | runtime | RUNTIME | rollback RESTORES prior content | A room is not a guest, and the screen now says which grace is actually in force |
| `hotel-admin/test/checkout-grace-effective.test.tsx` | CREATED | `A` | runtime | RUNTIME | rollback REMOVES it | A room is not a guest, and the screen now says which grace is actually in force |

## Total diff statistics (`git diff --stat`)
```text
 data-plane/cmd/edged/resources_phase3.go           |  70 ++++
 ..._reuse_and_lifecycle_matrix_integration_test.go | 355 +++++++++++++++++++++
 ...
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip | Bin 340192 -> 340190 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  | Bin 140436 -> 140442 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip | Bin 43885 -> 43885 bytes
 .../chatgpt/phase-evidence/GIT_STAT_cb8be71d.txt   |   4 -
 .../chatgpt/phase-evidence/GIT_STAT_d98da735.txt   |   4 +
 ...
 governance/transitions/T0144.json                  |  61 ++++
 .../components/phase3/checkout-grace-form.tsx      |  86 +++++
 hotel-admin/test/checkout-grace-effective.test.tsx | 158 +++++++++
 29 files changed, 863 insertions(+), 116 deletions(-)
```
...
D  exports/chatgpt/phase-evidence/GIT_STAT_cb8be71d.txt
A  exports/chatgpt/phase-evidence/GIT_STAT_d98da735.txt
...
```text
HISTORICAL: d98da735 A room is not a guest, and the screen now says which grace is actually in force
```
