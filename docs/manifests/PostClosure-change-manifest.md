...
- **Base commit:** `5705afacdd22635fad9da2f32dc40c501b171724`
- **HEAD commit:** `e2f8ee21`
- **Provenance (generation HEAD = inventory_head):** `e2f8ee21ed4c4e9417a2a702c5534a36558353a6`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
- **Branch:** `fix/reproducible-appliance-builds`
- **Remote branch:** `(no upstream)`
- **Changed files:** 27
- **Generated by:** `tools/generate-change-manifest.py 5705afacdd22..STAGED`

## Files

| Path | Classification | Git status | Domain | Workstream | Rollback | Purpose (last commit subject in range) |
|---|---|---|---|---|---|---|
| `docs/DISASTER_RECOVERY_FACTORY_CLEAN_INSTALL.md` | MODIFIED | `M` | documentation | DOCS | rollback RESTORES prior content | The appliance build recipe lived in shell history, so it drifted |
...
| `exports/chatgpt/phase-evidence/GIT_STAT_055782a5.txt` | EXPORTED | `D` | export | EXPORT | rollback RESTORES it | (no commit subject in range) |
| `exports/chatgpt/phase-evidence/GIT_STAT_e2f8ee21.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | (no commit subject in range) |
...
| `governance/project-state.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | The appliance build recipe lived in shell history, so it drifted |
| `governance/transitions/T0138.json` | CREATED | `A` | governance | GOVERNANCE | rollback REMOVES it | The appliance build recipe lived in shell history, so it drifted |
| `scripts/build-appliance-binaries.sh` | CREATED | `A` | other | OTHER | rollback REMOVES it | The appliance build recipe lived in shell history, so it drifted |

## Total diff statistics (`git diff --stat`)
```text
 docs/DISASTER_RECOVERY_FACTORY_CLEAN_INSTALL.md    |  17 ++-
 ...
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip | Bin 340198 -> 340194 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  | Bin 139901 -> 139886 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip | Bin 43886 -> 43883 bytes
 ...
 governance/transitions/T0138.json                  |  83 ++++++++++++++
 scripts/build-appliance-binaries.sh                | 110 +++++++++++++++++++
 27 files changed, 331 insertions(+), 121 deletions(-)
```
...
D  exports/chatgpt/phase-evidence/GIT_STAT_055782a5.txt
A  exports/chatgpt/phase-evidence/GIT_STAT_e2f8ee21.txt
...
```text
HISTORICAL: e2f8ee21 The appliance build recipe lived in shell history, so it drifted
```
