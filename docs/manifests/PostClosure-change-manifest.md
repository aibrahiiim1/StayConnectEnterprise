...
- **Base commit:** `265e5c87cf9bd8eb3daec34b0917cc7e25c21c7c`
- **HEAD commit:** `6a4a9671`
- **Provenance (generation HEAD = inventory_head):** `6a4a96718669f3d663594c74181cb9699afc9e56`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
- **Branch:** `fix/checkout-grace-durability-tests`
- **Remote branch:** `(no upstream)`
- **Changed files:** 26
- **Generated by:** `tools/generate-change-manifest.py 265e5c87cf9b..STAGED`

## Files

| Path | Classification | Git status | Domain | Workstream | Rollback | Purpose (last commit subject in range) |
|---|---|---|---|---|---|---|
| `data-plane/internal/checkout/grace_window_durability_integration_test.go` | CREATED | `A` | tests/tooling | RUNTIME | rollback REMOVES it | The grace clock belongs to the row, and now a test says so |
...
| `exports/chatgpt/phase-evidence/GIT_STAT_04f34cf9.txt` | EXPORTED | `D` | export | EXPORT | rollback RESTORES it | (no commit subject in range) |
| `exports/chatgpt/phase-evidence/GIT_STAT_6a4a9671.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | (no commit subject in range) |
...
| `governance/project-state.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | The grace clock belongs to the row, and now a test says so |
| `governance/transitions/T0142.json` | CREATED | `A` | governance | GOVERNANCE | rollback REMOVES it | The grace clock belongs to the row, and now a test says so |

## Total diff statistics (`git diff --stat`)
```text
 .../grace_window_durability_integration_test.go    | 156 +++++++++++++++++++++
 ...
 docs/manifests/PostClosure-change-manifest.md      |  88 ++++++------
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip | Bin 340193 -> 340186 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  | Bin 140443 -> 140428 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip | Bin 43883 -> 43884 bytes
 ...
 governance/transitions/T0142.json                  |  65 +++++++++
 26 files changed, 330 insertions(+), 101 deletions(-)
```
...
D  exports/chatgpt/phase-evidence/GIT_STAT_04f34cf9.txt
A  exports/chatgpt/phase-evidence/GIT_STAT_6a4a9671.txt
...
```text
HISTORICAL: 6a4a9671 The grace clock belongs to the row, and now a test says so
```
