...
- Appliance: the retired development reference appliance (SSH as root, key auth), code at `/opt/stayconnect`, Postgres in container `stayconnect-pg`, site DB `stayconnect_site` (+ an isolated second-site **test** DB `stayconnect_site_b` used for isolation tests — **not** a replication standby).
