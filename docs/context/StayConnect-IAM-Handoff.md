...
1. **Product-Owner review of the Phase-1A SCRATCH acceptance evidence** (`iam_v2_scratch/EVIDENCE.txt`; 51/51 A-series + 6/6 isolation).
2. **Then a separate, explicit Product-Owner authorization to create dark `iam_v2` in the live `stayconnect_site` database** (approval-ladder step 5) — scratch acceptance does **not** auto-authorize live-dark creation.
3. Live-dark creation does **not** authorize Phase 1B or cutover; **cutover** is a still-later, separately gated event (plan §7a/§11). Each transition needs its own PO approval.
