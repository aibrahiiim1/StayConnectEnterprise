...
- The Phase 1A execution plan ([StayConnect-IAM-Phase1A-Plan.md](../architecture/StayConnect-IAM-Phase1A-Plan.md)) is revised and **`READY_FOR_PRODUCT_OWNER_IMPLEMENTATION_APPROVAL`** — **planning only, not started**. Isolation is an **isolated `iam_v2` schema** inside the existing site DB (dark; rollback = leave dark / drop schema; a separate gated `search_path` cutover — **not** a whole-DB blue/green swap). Row-level locks preferred over advisory (verified admission namespaces `LN_DEVICE_SLOT`=11 / `LN_CAPACITY`=7 from `session.go`); cutover is an **atomic complete-domain switch of all IAM services** (never per-flow/per-service); reversal stays `capability=false` (no executable reversal built in 1A); Stripe FK deferred (no platform anchor exists). **Open item (BLOCKER for the folio gate only):** the plan's fail-closed `folio_identity_strategy` (unset ⇒ block posting) contradicts the FINAL contract §4.1 DDL (`NOT NULL DEFAULT 'GLOBALLY_UNIQUE'`) and needs a **separate Product-Owner-approved contract amendment**; until then 1A uses the DDL as written. It does not block approving the rest of the plan.
...
- **HA synchronization transport under the two-NIC rule.** The prior HA design (VRRP/conntrackd/Postgres streaming replication) assumed a **dedicated third `hasync` NIC**, which is now superseded. The exact HA-sync transport over a two-NIC (WAN+LAN) appliance is **not defined or implemented** — this is an **OPEN architecture decision**. Do **not** claim any WAN/LAN HA transport is implemented; HA itself remains a documented, not-yet-implemented limitation.
- **`folio_identity_strategy` fail-closed amendment** — see the Phase-1A plan §9 BLOCKER; requires a Product-Owner-approved contract §4.1 DDL amendment.
