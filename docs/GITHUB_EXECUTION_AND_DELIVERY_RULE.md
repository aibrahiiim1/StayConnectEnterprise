...
**Repository-side enforcement (GH-MANDATORY-CI).** GitHub Actions is the authoritative merge gate. The workflow `.github/workflows/project-governance.yml` (workflow name **Project Governance**, job **governance**) runs on `pull_request` to `master` and on every push to `master`, where a daytime pull-request attempt executes only the cheap sentinel and the full gate executes on the nightly re-run of that same run (see [NIGHTLY_AUTHORITATIVE_DELIVERY.md](NIGHTLY_AUTHORITATIVE_DELIVERY.md)); it checks out full history (`fetch-depth: 0`), runs `python tools/project-state.py validate`, `python tools/project-state.py check-generated`, `python tools/tests/project_state_validator/run_mutations.py` and `bash tools/validate-project-state.sh`, and then asserts the working tree is still clean — failing on any non-zero step. It uses read-only repository permissions and performs no deployment, database access, migrations or production tests. **A PR is not merge-ready until this check is green** — local validator output alone is insufficient for a delivered PR. The default branch should be protected to require the `governance` check.
...
- `tools/validate-delivery-protocol.py` — the **Fast Delivery and Parallel-Agent protocol**, asserted from the tree: every required gate runs on `pull_request`, declares no `workflow_dispatch`, carries the daytime sentinel as its first step (attempt 1 only, ending in `exit 1`), forbids evidence reuse on the authoritative re-run attempt, runs on no push but master, and carries a `concurrency:` block that cancels superseded pull-request runs while never cancelling a master run; the nightly runner re-runs each gate's `pull_request` run rather than dispatching anything; and the preflight, its protocol document and the PR template all exist and still implement what they claim. Adversarially proven by `run_mutations.py` cases M61-M65, M61f-M61g.
