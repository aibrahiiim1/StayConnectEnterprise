...
netd authorizes in phase3_auth_ipv4        → [ACCESS on a SHORT PROVISIONAL LEASE, METERED]
netd verifies authorization
netd promotes via the controlled writer    → Session = active  (now a true statement)
netd extends the lease to its full length  → [ACCESS on the full lease, METERED]
```

The lease lengths are the design, not a tuning knob:
...
**Renewal is a delete+add in one nft transaction, not a repeated `add element`.** nftables does not restart an
existing element's timer on a second add, so a renewal built on `add` would look healthy and still drop every
guest at the first lease boundary. That is verified against a real kernel, not assumed.
...
And underneath all of it, the lease: if acctd dies, and netd dies, and nothing recovers, every Phase-3 guest
loses internet access within 90 seconds of their last successful reconciliation — because the kernel expires
the element with nothing running at all. That is the only mechanism in this design that does not depend on
some process still being alive, which is why packet authorization is the one thing that uses it.
...
| `phase3_auth_ipv4` | exists, **empty**, nothing writes it | netd writes and reconciles it, always as a bounded lease |
...
### Getting the foundation onto an appliance that is already running

"Rendered always, starts empty" is true of a ruleset generated by the current build. It is **not** true of the
appliance in the field, which is running an older ruleset that has no Phase-3 set and whose captive DNAT rules
exclude only `auth_ipv4`. Getting from there to here by regenerating and re-applying the ruleset is exactly
the `delete table` + recreate that would flush `auth_ipv4` and drop every live legacy guest.

So the foundation is installed **surgically**, by `cmd/phase3-foundation`, in ONE nft transaction that adds a
set, adds one forward rule, and rewrites the captive rules in place. Nothing is flushed. The operation
snapshots the legacy authorization elements before it runs, re-reads them afterwards, and rolls itself back if
they are not exactly as they were. It is idempotent, it has an explicit rollback that touches nothing legacy,
and it authorizes nobody: the set it creates is empty and stays empty until the flags are turned on.

**Until that install has actually been performed and verified on the appliance, the cutover is not
"flag-only".** It is flag-only *afterwards*. This document does not claim otherwise, and neither should the
runbook or the report.
...
14. Phase-3 packet authorization is **never permanent**. Every element carries a bounded lease, renewed only
    by a healthy reconciliation and never extended past the session's own hard access boundary.
15. A Session is promoted to `active` only by a controlled operation that **verifies the accounting origin
    itself** — the checkpoint for this session, this device, this bridge, this class minor and this exact
    generation. "ACTIVE means authorized and accountable" is a database invariant, not an ordering convention.
16. An activation whose durable outcome cannot be proven holds only the provisional lease, and past the
