...
<!-- source: governance/project-state.json (schema 1.0.0) @ transition T0176 -->
...
| Phase 0 | **CLOSED** |
| Next authorized activity | **See the generated PROJECT STATE block** — this contract does not carry a current next action. *(HISTORICAL, as of the Phase-0 FINAL approval on 2026-07-16, the next authorized activity was Product-Owner acceptance of Phase 1A — production `iam_v2` live-dark created and verified 2026-07-16, dark/not cut over, see [Live-Dark Acceptance](../acceptance/StayConnect-IAM-Phase1A-Live-Dark-Acceptance.md) — then Phase 1B planning under separate authorization. Phases 1A, 1B, 2, 3 and 4 have all since been accepted and closed.)* |
...
**Phase 0 is FINAL and CLOSED.** **HISTORICAL operational status, AS OF 2026-07-16 — superseded, NOT the current state:** Phase 1A production `iam_v2` was live-dark created, verified, and formally Product-Owner ACCEPTED and CLOSED at its DARK maturity (dark, isolated schema; no service read or wrote it, no DSN/`search_path` change, no IAM data migration, no Phase 1B implementation, no cutover), Phase 1B planning was the activity, and the next authorized activity was Product-Owner approval or rejection of the complete Phase 1B plan. **The current phase, activity and next action are the generated PROJECT STATE block; this contract carries none.**

> **[HISTORICAL — Phase-0 finalization note, superseded by the current status above]** At the moment Phase 0 closed (2026-07-16, before the live-dark build), the then-next activity was *Phase-1A planning only*, and FINAL status closed the Phase-0 architecture without by itself unlocking implementation. That gate has since been satisfied: the Phase 1A plan was approved and the live-dark schema was created and verified. This note is retained for audit only and does **not** describe current status.
...
**Finalization — DONE (2026-07-16):** the product owner gave **explicit FINAL approval** of this corrected contract; Phase 0 is **FINAL and CLOSED**. Tier-2 (per-property) and Tier-3 (post-implementation) items were **not** finalization blockers — they are, respectively, deployment prerequisites and binding acceptance requirements that carry forward past FINAL. Deferred limitations are listed in §9d. **HISTORICAL operational status, AS OF 2026-07-16 — superseded:** Phase 1A was scratch-verified, offline-real-schema-verified, and production `iam_v2` live-dark created + verified (dark, not cut over); the next authorized activity was Product-Owner acceptance of Phase 1A, then Phase 1B planning under separate authorization. **The current state is the generated PROJECT STATE block.**
...
| **1B Credential/Identity/Auth-Context (DARK)** | Auth contexts + voucher (HMAC/AEAD), account, OTP/social (guest principals) credential validation + identity/device resolution against `iam_v2`, **DARK / flags-OFF**; the session-after-grant adapter interface (scratch-tested only). **No guest-visible activation, no authority switch, no `search_path`/DSN cutover; no real production purchases/entitlements/`iam_v2` sessions** (those are Phase-2/full-domain). Least-privilege / superuser-elimination prerequisite (Gate P) first. | B-series **credential/identity/auth-context** parts (B1, B2, B5, B6; B3/B4 principal-resolution) — the entitlement/session parts of B3/B4 are **deferred to Phase-2 / full-domain acceptance**; plus flags-OFF non-regression + reboot/offline drills | flags OFF / DSN revert to legacy (no `iam_v2` writes in 1B) |
| **1B→cutover (later, separate gate)** | The atomic complete-domain cutover (all IAM services' `search_path`/DSN flipped to `iam_v2` together) is a **later separately approved gate — only after Phases 2–6 and full-domain acceptance**, never a Phase-1B credential-only cutover | full-domain B/C/D/E/F acceptance + explicit PO cutover approval | flip `search_path` back before first production write (Boundary A); after first write reverse-migrate / forward-fix (Boundary B) |
...
**End of contract.** Status: **FINAL — Phase 0 CLOSED** (Product-Owner approval, 2026-07-16). The Phase-0 protocol/architecture validation is complete and merged (§9b/§9c). Per-property financial onboarding (§9c Tier 2, incl. Aqua Club) and post-implementation acceptance (§9c Tier 3 / §9d) carry forward as deployment prerequisites and binding acceptance requirements, not finalization blockers. **HISTORICAL operational status, AS OF 2026-07-16 — superseded:** Phase 1A production `iam_v2` was live-dark created + verified (dark, not cut over; no service routed, no DSN/`search_path` change), and the next authorized activity was Product-Owner acceptance of Phase 1A, then Phase 1B planning under separate authorization. *(The Phase-0 design/architecture above is FINAL and unchanged. For the current phase, activity and next action see the generated PROJECT STATE block — this contract carries none.)*
