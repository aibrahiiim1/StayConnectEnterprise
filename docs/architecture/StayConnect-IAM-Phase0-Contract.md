...
**Single next authorized action:** AWAIT A SEPARATE PRODUCT-OWNER DECISION TO AUTHORIZE LIVE INCREMENT 9. The Phase-3 SOFTWARE candidate is complete: Option C (D15) executed with no scope reduction, the mixed Phase-2 direct-Entitlement writer eliminated, staged accountable-before-forwarding class provisioning in place, and one true full same-HEAD software gate (phase3-full-software-gate) green with a downloadable, independently-verified evidence artifact. Phase 3 is NOT accepted and NOT closed; PR #6 is open and unmerged; all Phase-3 flags are OFF; migration 0010 is undeployed. The next authorized action is NOT to "execute Phase 3 implementation" (that is done) and NOT to accept Phase 3: it is a distinct Product-Owner decision that would authorize Live Increment 9 (read-only live PMS verification, controlled live-dark deployment of the exact delivery HEAD, one reboot with post-reboot convergence, a rollback rehearsal, and flags-OFF confirmation on the running unit). No appliance/Production-DB/live-PMS contact, deployment, reboot, Gate-P grant, PS/PA, financial posting, or Phase 4 is authorized until then.
...
| Phase 0 | **CLOSED** |
| Next authorized activity | **Product-Owner acceptance of Phase 1A** (production `iam_v2` **live-dark created and verified 2026-07-16**, dark/not cut over — see [Live-Dark Acceptance](../acceptance/StayConnect-IAM-Phase1A-Live-Dark-Acceptance.md)), then **Phase 1B planning under separate authorization**. Cutover, service routing, and IAM data migration remain separately gated. |
...
**Phase 0 is FINAL and CLOSED.** **Current operational status (2026-07-16):** Phase 1A production `iam_v2` is **live-dark created, verified, and formally Product-Owner ACCEPTED and CLOSED at its DARK maturity** (dark, isolated schema; **no service reads/writes it, no DSN/`search_path` change, no IAM data migration, no Phase 1B implementation, no cutover**). **Phase 1B planning is the current activity; next authorized activity: Product-Owner approval or rejection of the complete Phase 1B plan** (Phase 1B implementation, cutover, service routing, IAM data migration, deployment, and legacy cleanup remain separately gated).
