...
CREATE TABLE pms_interface_revisions (                        -- append-only (trigger-enforced)
  ...
  folio_identity_strategy text NOT NULL DEFAULT 'GLOBALLY_UNIQUE'
...
CREATE UNIQUE INDEX folio_open_identity ON folios (tenant_id, site_id, pms_interface_id, external_folio_id)
  WHERE status='OPEN';
-- Folio identity strategy (per interface revision): GLOBALLY_UNIQUE keeps epoch=1 forever;
...
2. **Protocol-attempt ledger (`posting_attempts` + `posting_attempt_events`).** DDL in §4.5.
   ...
   - **`P#` allocation** is a **durable atomic per-interface sequence** (`pms_interface_pnumber_seq`, bumped transactionally) — **not** a Unix timestamp. Uniqueness and ownership are scoped by **`tenant_id, site_id, pms_interface_id, p_number`**.
...
- **PMS Posting:** `PENDING → SENDING → POSTED | FAILED_RETRYABLE | FAILED_FINAL | UNKNOWN`; `UNKNOWN → MANUAL_REVIEW → POSTED | FAILED_FINAL`; reversal is a new REVERSAL row. Connectors without idempotency/read-back never auto-retry from UNKNOWN.
...
**E. Financial:** E1 full pin-chain fuzz (quote tuple, payment parent scope, watermark tenancy, posting pair) rejected at the SQL layer · E2 idempotency-key race → one charge · E3 UNKNOWN → manual review; all five governance actions with re-auth/reason/evidence; dual approval threshold · E4 posting on non-IN_HOUSE/blocked stay aborts · E5 posting permission evaluation recorded (no-post flag, closed folio, admin block, credit policy); IN_HOUSE alone grants nothing · E6 folio change between auth and purchase handled (folio pinned at purchase) · E7 secret generations pinned; delete refused until drained · E8 outbox one-active-row; retries never change interface · E9 duplicate-source severity tiers; only CRITICAL gates posting · E10 interface outage isolation · E11 decommission guards + audited override · E12 folio-number reuse: recycled number ⇒ new identity_epoch; history unaffected; open-folio uniqueness held.
