...
**Fresh Production appliance (172.21.60.25, sce):** DEPLOYED factory-clean from `c72b49e`; first-bring-up fixes were then applied live and afterwards committed to master (`1c55d7f`), so no single commit describes what is running. PRE-LIVE. Enrollment, claim and signed assignment are NOT complete and it is unlicensed; no tenant or site is pinned. LAN is intentionally unconfigured. No guest, PMS, payment or financial traffic; no Go-Live. Hotel Admin: https://172.21.60.25/
...
**Single next authorized action:** MANUAL APPLIANCE ONBOARDING on the deployed Fresh Production appliance 172.21.60.25, beginning with ENROLLMENT. The server is provisioned and running; provisioning is no longer the next action. In order: (1) issue a bootstrap token from the Central Control Plane and enrol the appliance through the local setup wizard; (2) claim it and let the SIGNED ASSIGNMENT resolve its tenant and site -- no identity may be seeded or copied; (3) install the signed licence; (4) configure LAN/guest networking through Hotel Admin, at which point Kea starts; (5) configure packages, access policy and PMS interfaces; (6) acceptance test. A separate Product-Owner Go-Live decision remains required, and this state authorizes no guest, PMS, payment or financial traffic. 172.21.60.23 remains the DEVELOPMENT reference appliance and is not to be changed.
