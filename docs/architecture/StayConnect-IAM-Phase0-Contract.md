...
**Phase 0 is FINAL and CLOSED.** **Current operational status (2026-07-16):** Phase 1A production `iam_v2` is **live-dark created and verified** (dark, isolated schema; **no service reads/writes it, no DSN/`search_path` change, no IAM data migration, no Phase 1B, no cutover**). **Next authorized activity: Product-Owner acceptance of Phase 1A, then Phase 1B planning under separate authorization** (cutover, service routing, IAM data migration, deployment, and legacy cleanup remain separately gated).
