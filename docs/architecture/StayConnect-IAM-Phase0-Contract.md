...
**Single next authorized action:** NONE is authorized. The interface-presence correction is complete and its fail-open is closed: guest network validation observes presence live at the moment of the decision and refuses when it cannot (PR #99, merged as 3984ed41), proven through netd's real /v1/validate endpoint on PRE-LIVE with a created-then-deleted interface. No further action follows: no Room Login, no PMS configuration change, no Full Resync, no financial traffic, no further redeployment, no migration, no service restart, no Stay modification and no Go-Live is authorized. PRE-LIVE remains PRE-LIVE.
