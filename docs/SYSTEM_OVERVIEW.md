...
**Caddy operational gotchas (from the pilot):** `chown -R caddy:caddy /var/log/caddy` after first run (root-owned logs kill the service); disable the stock apt `caddy.service` (port-443 race with `stayconnect-caddy`); `/etc/caddy/Caddyfile` is a REAL FILE installed from the role-specific source (`deploy/caddy/Caddyfile.edge` on the appliance, `deploy/caddy/Caddyfile.central` on Central) - it is not a symlink into the deploy tree, and the Edge config carries no Central vhosts (it declares `api.`/`admin.` only to refuse them with 404). The systemd unit is role-specific for the same reason: `deploy/caddy/stayconnect-caddy.edge.service` supports `systemctl reload`, while `deploy/caddy/stayconnect-caddy.central.service` deliberately has NO `ExecReload` because Central runs with `admin off` - apply config there with validate + `systemctl restart`; on the VM `/etc/hosts` points the three hostnames at **10.10.0.1** (not 127.0.0.1) so network-namespace tests can reach them; Caddy's root CA is at `/var/lib/caddy/.local/share/caddy/pki/authorities/local/root.crt` (or use `curl -k`).
