...
- **Canonical migration directory (§2):** dir resolved with `realpath` and compared to the checked-out repo path; arbitrary `--dir` refused unless a separately-acknowledged disposable test mode; symlink/escaped dir yields no migration; symlinked migration file rejected.
- **Checksum (§4):** SHA-256 computed and compared to mandatory `--expect-sha256` before the lock; mismatch refused, printing expected + actual (no credentials). For a not-yet-applied migration (0010 on the appliance) this pre-apply SHA is the binding control; a skipped legacy ledger row is NOT claimed to carry a DB-recorded checksum; the live ledger schema is not silently altered to add one.
- **Structural ledger verification before the lock (§3):** `schema_migrations` exists; `version` is text NOT NULL PRIMARY KEY; `applied_at` is timestamptz NOT NULL; no duplicate versions; owner in the approved allowlist; the accepted `0009_phase2_commerce` baseline is present before 0010. Verified: unexpected column type/nullability/PK, unexpected owner, and missing 0009 all refused; the apply/skip decision still happens only under the advisory lock.
- **Apply vs rollback role split (§4):** the forward-**apply** role needs exactly `SELECT` (read the ledger) + `INSERT` (record the applied version) — it does **not** need or hold `DELETE`. In `live-site` mode the apply role is additionally proven to hold **no** `UPDATE`/`DELETE`/`TRUNCATE`/`REFERENCES`/`TRIGGER` on the ledger and **no** public `CREATE`. Ledger removal / the down migration is a separate rollback/admin operation with its own authorization — never part of a forward apply. Gate verifies the apply role (`iam_v2_owner`) holds `SELECT`+`INSERT` and none of the destructive rights.
- **Separated ledger bootstrap (§5):** `--bootstrap-ledger` is standalone — cannot combine with `--only`/`--all`, requires its own `--expect-db`/`--target-kind`/`I_UNDERSTAND_LEDGER_BOOTSTRAP`/`--bootstrap-owner`, creates + owns + verifies the ledger, and applies NO migration. Normal mode never creates or alters the ledger.
- **Deployment-parity roles (§6):** in addition to the NOSUPERUSER `iam_v2_owner` apply, the disposable parity setup creates `svc_scd`/`svc_edged`/`svc_portald`/`svc_acctd`/`svc_pmsd` and proves each has ZERO iam_v2 table/function privileges while DARK; the migration role holds no broad public CREATE/DDL; no default privileges leak future iam_v2 objects to PUBLIC/service roles; all new 0010 objects owned by `iam_v2_owner`. (The ownership reassignment is disposable test scaffolding — Migration 0010 does not reassign every object owner on the future appliance.)

Part-A hardening additions (earlier pass):
