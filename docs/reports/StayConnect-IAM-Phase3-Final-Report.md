...
- **HEAD commit:** `11411b9`
- **Provenance (generation HEAD = inventory_head):** `11411b9f44dda65250e9f1ac89f1c8946c7e95ed`  ·  path/status set covers the complete `base..delivery_head` diff (delivery_head = this staged content once committed).
...
| `deploy/env/pmsd.env.dark` | CREATED | `A` | configuration | DEPLOY | rollback REMOVES it | Phase 3 DARK acceptance (inventory_head): a rollback that cannot preserve authorization now refuses, and pmsd is actually deployed |
...
| `docs/reports/StayConnect-IAM-Phase3-Final-Report.md` | CREATED | `A` | documentation | DOCS | rollback REMOVES it | Phase 3 DARK acceptance (inventory_head): a rollback that cannot preserve authorization now refuses, and pmsd is actually deployed |
...
| `exports/chatgpt/phase-evidence/GIT_STAT_11411b9.txt` | EXPORTED | `A` | export | EXPORT | rollback REMOVES it | (no commit subject in range) |
...
| `governance/project-state.json` | MODIFIED | `M` | governance | GOVERNANCE | rollback RESTORES prior content | Phase 3 DARK acceptance (inventory_head): a rollback that cannot preserve authorization now refuses, and pmsd is actually deployed |
...
| `scripts/phase3-preflight.sh` | CREATED | `A` | other | OTHER | rollback REMOVES it | Phase 3 DARK acceptance (inventory_head): a rollback that cannot preserve authorization now refuses, and pmsd is actually deployed |
...
```text
 ...
 docs/manifests/Phase3-change-manifest.md           |  815 ++++++
 .../reports/StayConnect-IAM-Phase2-Final-Report.md |    4 +-
 .../reports/StayConnect-IAM-Phase3-Final-Report.md | 1577 +++++++++++
 .../StayConnectEnterprise-ChatGPT-Project-Pack.zip |  Bin 250675 -> 289911 bytes
 .../StayConnectEnterprise-Phase-Evidence-Pack.zip  |  Bin 101471 -> 107791 bytes
 ...StayConnectEnterprise-Phase1B-Planning-Pack.zip |  Bin 41921 -> 42129 bytes
 .../chatgpt/phase-evidence/GIT_STAT_11411b9.txt    |    4 +
 ...
 scripts/phase3-preflight.sh                        |  691 +++++
 ...
 273 files changed, 55909 insertions(+), 557 deletions(-)
```
...
A  exports/chatgpt/phase-evidence/GIT_STAT_11411b9.txt
D  exports/chatgpt/phase-evidence/GIT_STAT_8e4a45b.txt
