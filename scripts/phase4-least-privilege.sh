#!/usr/bin/env bash
# Self-contained least-privilege gate: builds a disposable PG16 carrying the full chain through 0017 and
# runs the restricted-role proof. Disposable infrastructure only; no Production or appliance access.
# EXIT: 0 pass, 1 assertion failure (never retry), 2 infrastructure (retryable).
set -uo pipefail
export PATH="$PATH:/c/Program Files/Docker/Docker/resources/bin"
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
C="${PHASE4_LP_CONTAINER:-iamv2-p4lp}"; DB=iam_scratch; PORT="${PHASE4_LP_PORT:-55441}"
...
for m in 0009_phase2_commerce 0010_phase3_stay_resolution 0011_phase4_financial_execution \
         ...
         0015_phase4_payment_hardening 0016_phase4_payment_coherence 0017_phase4_least_privilege 0018_phase4_financial_identity_and_privilege 0019_phase4_financial_recovery 0020_phase4_financial_observability 0021_phase4_trust_boundary 0022_phase4_recovery_closure 0023_phase4_restore_generation; do
  if ! docker exec -i "$C" psql -U postgres -d "$DB" -v ON_ERROR_STOP=1 \
    ...
    docker exec -i "$C" psql -U postgres -d "$DB" -v ON_ERROR_STOP=1 \
      < "$ROOT/data-plane/migrations/$m.up.sql" 2>&1 | tail -5
    exit 1
  fi
done
...
PHASE4_LP_CONTAINER="$C" PHASE4_LP_DB="$DB" bash "$ROOT/iam_v2_scratch/phase4_least_privilege.sh"
