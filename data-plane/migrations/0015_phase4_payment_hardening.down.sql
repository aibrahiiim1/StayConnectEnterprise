-- 0015 DOWN — reverse of 0015_phase4_payment_hardening.up.sql. Reverses ONLY 0015.
--
-- Rolling back restores 0014's weaker posture, including its SELECT-then-decide bounds. That is strictly
-- less safe, which is why this exists as a rollback path and not an operating one.
BEGIN;

DELETE FROM public.schema_migrations WHERE version = '0015_phase4_payment_hardening';

DROP FUNCTION IF EXISTS iam_v2.apply_payment_callback_v2(uuid,text,uuid,text,text,text,text,text,jsonb);

DROP INDEX IF EXISTS iam_v2.ptx_event_provider_identity;
ALTER TABLE iam_v2.payment_transaction_events
  DROP CONSTRAINT IF EXISTS ptx_event_detail_safe,
  DROP CONSTRAINT IF EXISTS ptx_event_ids_bounded,
  DROP COLUMN IF EXISTS merchant_account_id,
  DROP COLUMN IF EXISTS provider;

DROP FUNCTION IF EXISTS iam_v2.p4_callback_evidence_safe(jsonb);

DROP INDEX IF EXISTS iam_v2.ptx_one_live_charge_per_settlement;
DROP INDEX IF EXISTS iam_v2.ptx_provider_txn_ref_identity;
ALTER TABLE iam_v2.payment_transactions
  DROP CONSTRAINT IF EXISTS ptx_local_ref_bounded,
  DROP CONSTRAINT IF EXISTS ptx_provider_txn_ref_bounded,
  DROP COLUMN IF EXISTS intent_created_at,
  DROP COLUMN IF EXISTS provider_txn_ref;

DROP FUNCTION IF EXISTS iam_v2.ns_payment_parent(text);

-- restore the 0014 callback entry point and the 0014 creation/status/settlement bodies
-- These MUST be restored: 0015's bodies reference columns this rollback drops, so leaving them would
-- break the table rather than merely relax it.
...
BEGIN
  IF p_provider_event_id IS NULL OR btrim(p_provider_event_id) = '' THEN
    RAISE EXCEPTION 'CALLBACK_EVENT_ID_REQUIRED' USING ERRCODE = 'check_violation';
  END IF;
  ...
  EXCEPTION WHEN unique_violation THEN
    v_dup := true;
  END;
  IF v_dup THEN RETURN 'DUPLICATE'; END IF;
  IF NOT v_moves THEN RETURN 'NOOP'; END IF;
