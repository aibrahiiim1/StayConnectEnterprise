-- PHASE 6 M4 — an exhaustion instant must be PROVABLE, not assumed.
--
-- THE STATE IN QUESTION. An AGGREGATE_ONLINE_TIME entitlement whose consumed_online_seconds has reached the
-- budget while online_time_exhausted_at is NULL. The tick stamps that instant inside the tick that crosses
-- it, so the ordinary path always has one -- but two supported paths can produce consumption at budget with
-- no stamp:
--
--   * an audited adjustment (iam_v2.entitlement_adjustments) that raises consumption or lowers the effective
--     budget. The counter guard makes adjustments the ONLY way a counter moves other than by accrual;
--   * a restore or import that carries the counter forward without the stamp.
--
-- WHAT THE CODE USED TO DO. Both the tick and p6_due_terminal fell back to `now()`. That writes a HISTORICAL
-- FACT that nobody observed: the entitlement is recorded as having run out at the moment a sweep happened to
-- look, and everything attributed to that instant -- the termination, the evidence row, the session end times
-- -- inherits the invention. It is the same defect class as stamping a data crossing at sweep time, and it is
-- worse here because there is no sample to appeal to afterwards.
--
-- WHAT IT DOES NOW. The instant is derived from evidence or it does not exist:
--
--   1. the stamp, when the tick computed one -- the authoritative case;
--   2. otherwise the created_at of the earliest audited adjustment that could have produced the state. That
--      is provable: the row says the counter moved, and when;
--   3. otherwise NULL, and NOTHING claims exhaustion. No stamp is written, no terminal condition is
--      reported, no evidence row is created.
--
-- CASE 3 IS DELIBERATELY NOT A TERMINATION. Ending a guest's access needs a defensible instant, and "we
-- noticed" is not one. The entitlement remains subject to every other terminal condition -- its outer window,
-- its data quota -- and the next accrual tick that observes real online time will cross the budget properly
-- and stamp it. The state is also visible: it is exactly what the operator budget view shows as consumed at
-- or over budget while still active, and the function warns so the anomaly reaches the log rather than
-- nobody.
BEGIN;
...
COMMENT ON FUNCTION iam_v2.p6_exhaustion_instant(uuid) IS
  ...
  'audited adjustment that moved the counter, else NULL. Never now(): a historical fact nobody observed '
  'would be inherited by the termination, the evidence and every session end time attributed to it.';

-- p6_due_terminal now asks that function instead of assuming.
...
-- ---------------------------------------------------------------------------------------------------------
-- The accrual tick, for the same reason: its already-exhausted branch fabricated an instant too.
-- ---------------------------------------------------------------------------------------------------------
...
COMMENT ON FUNCTION iam_v2.p6_tick_online_time(uuid, uuid, timestamptz, int, uuid[], timestamptz[]) IS
  'One AGGREGATE_ONLINE_TIME accrual tick for a site. Billable only to the earliest of now, session end, the '
  'immutable outer window and any cap the caller passes; charges only sessions in state active, within the '
  'per-tick observation bound. Exhaustion is the exact piecewise crossing, and an already-exhausted '
  'entitlement is re-reported only with an instant that can be proven -- never with the clock of the tick '
