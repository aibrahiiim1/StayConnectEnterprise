-- 0088 — A VOUCHER GRANTS WHAT IT WAS PRINTED FOR.
--
-- iam_v2.vouchers.package_revision_id is NOT NULL and has been pinned at issuance since mg3 defined the
-- table. The issuance path states the reason in its own comment: "what a voucher grants is fixed at
-- issuance by an immutable revision, so republishing a package later cannot retroactively change what an
-- already-printed card is worth". The Hotel Admin screen this delivery added says the same thing to the
-- operator, in so many words: "What these cards grant".
--
-- NOTHING ENFORCED IT.
--
-- iam_v2.auth_contexts carries voucher_id and has no package pin at all (mg5). So a VOUCHER auth context
-- reached ListEligiblePackages and CreateQuote with nothing to narrow it, and those offer every package the
-- subject is eligible for. At a property with two voucher-eligible free tiers, a card printed for the
-- lower one could be redeemed against the higher -- and both the code comment and the operator screen
-- asserted that this could not happen. A claim made in two places and enforced in none is worse than no
-- claim: it is the reason nobody checks.
--
-- WHY THE KERNEL. p4_entitlement_grant_kernel is the single place both grant entry points funnel through
-- (p4_grant_quoted_entitlement for a free quote, p4_grant_paid_entitlement for a settled one), it already
-- receives BOTH p_voucher and p_pkg_rev, and it is SECURITY DEFINER owned by iam_v2_owner -- so the check
-- cannot be routed around by any caller, present or future. Migration 0084 put the single-use burn here
-- for exactly these reasons; this is the same argument about a different invariant.
--
-- WHY BEFORE THE LOCK. The check runs before the subject advisory lock and before any write, so a grant
-- that is going to be refused does not first terminate the guest's previous entitlement on its way to
-- failing. 0084's burn stays where it is, after the insert, because it must be in the same transaction as
-- the entitlement it spends the card for.
--
-- THE OFFER PATH NARROWS TOO, in the Go layer, so an operator or guest never sees a choice this would
-- refuse. That is product behaviour and it lives where behaviour lives; this is the invariant, and it holds
-- whether or not the layer above is correct.
--
-- NO PRIVILEGE CHANGES. The kernel already reads iam_v2.vouchers as its owner. svc_scd gains nothing, and
-- 0084's assertion that it still cannot write voucher state is repeated at the end.
...
-- 0084's boundary, restated: the burn and now the pin check both live in the kernel precisely so that no
-- service role needs write access to voucher state.
DO $a$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'svc_scd') THEN
    IF has_table_privilege('svc_scd', 'iam_v2.vouchers', 'UPDATE') THEN
      RAISE EXCEPTION 'svc_scd holds UPDATE on iam_v2.vouchers; the single-use burn and the package pin are '
                      'enforced by the SECURITY DEFINER grant kernel and the service role must not be able '
                      'to write voucher state directly';
    END IF;
  END IF;
END;
$a$;
