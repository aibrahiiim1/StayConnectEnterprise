...
//
// It returns the full lease clamped by the session's hard access boundary, and false when the boundary has
// already passed — in which case the session must not be authorized at all. A caller that treated a
// non-positive lease as "no timeout" would install exactly the permanent element this file exists to prevent,
// so the boolean is the answer and the duration is never to be used without it.
func leaseFor(s shapeplan.Session, now time.Time) (time.Duration, bool) {
	ttl := phase3LeaseTTL
