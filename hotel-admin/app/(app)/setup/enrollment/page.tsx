...
// Activation now lives on the consolidated Appliance & licence screen.
//
...
export default function ActivationMoved() {
  const router = useRouter();
  useEffect(() => { router.replace("/appliance?section=setup"); }, [router]);
  return (
    <div className="mx-auto w-full max-w-3xl text-sm text-muted-foreground">
      Activation now lives on the <strong>Appliance &amp; licence</strong> screen. Taking you there&hellip;
    </div>
  );
}
