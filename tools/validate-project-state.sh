...
# The allowlist below must recognise the CURRENT next authorized action, and it lagged.
#
# T0083 replaced "Product-Owner provision of the NEW PRODUCTION SERVER" with "MANUAL ENROLLMENT of the
# deployed Fresh Production appliance" and the allowlist was not extended. The check kept passing only
# because the packed copies were still generated at T0082 and therefore still quoted the retired phrase --
# a stale artefact satisfying a check about staleness. Rebuilding the packs at T0085 surfaced it immediately.
#
# Adding the current phrase does not weaken anything: the rule is still that the packs must state a
# recognised next action, in at least two places, consistently. It is the vocabulary that was out of date,
# not the requirement.
#
# IT LAGGED AGAIN, and the second time cost a governance cycle. T0177 replaced "NONE is authorized" -- which
# contradicted the mission authorisation recorded in the same file -- with the FUNCTIONAL-COMPLETENESS
# CLOSURE action. Only one match remained in the packs, and that one was a HISTORICAL Phase-1B line, so a
# state file that had just been made truthful read as inconsistent. The two ways out were to extend this
# list or to write a phrase this list already knew; the second would have meant restoring the contradiction
# to satisfy a check about staleness, which is the exact failure the comment above describes.
#
# AND THE PREFLIGHT COULD NOT SEE IT. This validator runs in the governance gate and in the mutation
# suite's baseline, and the local preflight ran neither -- so the only place this rule could be learned was
# a runner, twenty minutes in. tools/preflight.sh stage 9 now runs it, and
# tools/validate-delivery-protocol.py requires that stage to exist, on the same footing as the four failures
# that were caught late before it.
