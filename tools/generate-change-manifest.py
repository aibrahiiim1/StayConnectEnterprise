...
def resolve_range(argv):
    """Return (base_sha, head_sha, verified[], out_path). Accepts the usage forms above."""
    verified, out = [], None
    pos = []
    i = 0
    while i < len(argv):
    ...
    return base_sha, head_sha, verified, out
...
def main():
    ...
    L.append(f"- **Generated by:** `tools/generate-change-manifest.py {base[:12]}..{head[:12]}`")
    L.append("")
    L.append("## Files")
    L.append("")
