...
MUTATIONS = [
 ...
 ("M04 two next authorized actions", "governance/project-state.json",
   ("replace", [('"Merge PR #4 to master with provenance preserved and run post-merge governance verification',
                 '"Merge PR #4 to master with provenance preserved; also enable the Phase-2 flags and run post-merge governance verification')])),
 ...
 ("M31 next action implies acceptance but allowed says execute Phase 1B", "governance/project-state.json",
   ("replace", [("Read-only verification and inspection of the accepted, closed, live-dark Phase 2 (commercial packages)",
 ...
 ("M35 closed/merged but an allowed_action still says merge PR #2", "governance/project-state.json",
   ("replace", [("Governance and documentation reconciliation only (no runtime code change, no migration, no appliance deploy/restart/reboot, no flag enablement)",
