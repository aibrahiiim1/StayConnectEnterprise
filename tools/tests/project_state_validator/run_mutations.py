...
MUTATIONS = [
 ...
 ("M04 two next authorized actions", "governance/project-state.json",
   ("replace", [('"next_authorized_action": "Complete Phase 2 execution end-to-end',
                 '"next_authorized_action": "Approve the plan; and also implement Phase 3. Complete Phase 2 execution end-to-end')])),
 ...
 ("M08 stale source commit / snapshot mismatch", "governance/project-state.json",
   ("replace", [('"latest_transition_id": "T0012"', '"latest_transition_id": "T0008"')])),
 ...
 ("M29 activity deployed but maturity says Gate P pending", "governance/project-state.json",
   ("replace", [("PHASE_2_IMPLEMENTATION_IN_PROGRESS", "PHASE_1B_LIVE_DARK_DEPLOYED_PENDING_PO_ACCEPTANCE"),
 ...
 ("M30 gate_p cutover done but blocker says superuser", "governance/project-state.json",
   ("replace", [("None. Phase 2 (commercial packages) implementation is IN_PROGRESS",
                 "Site-DB services still connect as superuser stayconnect; least-privilege roles are not yet applied and Phase 2 is IN_PROGRESS")])),
 ("M31 next action implies acceptance but allowed says execute Phase 1B", "governance/project-state.json",
   ("replace", [("Read-only inspection, documentation and governance work",
