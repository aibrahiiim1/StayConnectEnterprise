...
export function DialogHeader({ className, ...p }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("shrink-0 space-y-1 border-b border-border px-5 py-4 pe-12", className)} {...p} />
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !busy && onOpenChange(v)}>
      <DialogContent size={size}>
        ...
        <DialogFooter>
          {extraFooter && <div className="me-auto flex items-center gap-2">{extraFooter}</div>}
...
export function ConfirmDialog({
  ...
  confirmText?: string;
  /** What to call the typed value ("the site code"). Defaults to "Type X to confirm". */
    ...
    >
      ...
      {requireReason && (
        <Field label={reasonLabel} htmlFor={reasonId} required
          hint={reasonOptions ? "Recorded in the audit log." : `Recorded in the audit log. ${reason.length}/${reasonMaxLength}`}
        >
          {reasonOptions ? (
      ...
      {confirmText && (
        <Field
          label={
            confirmTextLabel ? (
              <>{confirmTextLabel}: <span className="font-mono font-bold">{confirmText}</span></>
            ) : (
