...
export default function SessionsPage() {
  ...
  async function load() {
    ...
    try {
      const r = await api.get<ListResp<Session>>(`/v1/sessions?${q.toString()}`);
      setRows(r.data);
    } catch (e: any) {
      setErr(e?.message ?? "Failed to load");
    }
  }
  useEffect(() => { load(); }, [tenantID, tab]);
  useEffect(() => {
  ...
  async function onDisconnect(sid: string) {
    ...
    try {
      await api.post(`/v1/sessions/${sid}/disconnect?tenant_id=${tenantID}`, { reason: "admin" });
      setTimeout(load, 500);
    } catch (e: any) {
