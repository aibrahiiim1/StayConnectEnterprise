...
import { api, ApiError, ListResp, TicketTemplate } from "@/lib/api";
...
export default function TicketTemplatesPage() {
  ...
  const [rows, setRows] = useState<TicketTemplate[] | null>(null);
  ...
  async function load() {
    if (!tenantID) return;
    try {
      const r = await api.get<ListResp<TicketTemplate>>(`/v1/ticket-templates?tenant_id=${tenantID}`);
      setRows(r.data);
  ...
  async function onCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await api.post(`/v1/ticket-templates?tenant_id=${tenantID}`, {
        code: form.get("code"),
        name: form.get("name"),
  ...
  async function onToggle(t: TicketTemplate) {
    if (!tenantID) return;
    try {
      await api.patch(`/v1/ticket-templates/${t.id}?tenant_id=${tenantID}`, { is_active: !t.is_active });
      load();
  ...
  async function onDelete(id: string) {
    ...
    try {
      await api.del(`/v1/ticket-templates/${id}?tenant_id=${tenantID}`);
      load();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-baseline justify-between mb-4">
        <div>
          ...
          <h1 className="text-2xl font-semibold">Ticket templates</h1>
        </div>
        <Button onClick={() => setShowNew((s) => !s)}>
          {showNew ? <><X size={14} /> Cancel</> : <><Plus size={14} /> New template</>}
        </Button>
      </div>
      ...
      <Card>
        <CardBody className="p-0">
          {rows === null ? <EmptyState title="Loading…" /> : rows.length === 0 ? (
            <EmptyState title="No templates yet" hint="Create one to start issuing voucher batches." />
          ) : (
            <Table>
              <THead>
                <TR>
