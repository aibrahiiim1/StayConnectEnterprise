...
import { api, ListResp, Voucher, VoucherBatch, Whoami } from "@/lib/api";
    ...
    (async () => {
      try {
        const [b, vs] = await Promise.all([
          api.get<VoucherBatch>(`/v1/voucher-batches/${id}?tenant_id=${tenantID}`),
          api.get<ListResp<Voucher>>(`/v1/voucher-batches/${id}/codes?tenant_id=${tenantID}&limit=500`),
        ]);
        setBatch(b);
        setVouchers(vs.data);
      } catch (e: any) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-baseline justify-between mb-4">
        ...
        <a
          href={`/api/v1/voucher-batches/${id}/codes.csv?tenant_id=${tenantID}`}
